...
export async function getFolderFiles(folderId: string): Promise<DriveFile[]> {
  try {
    ...
    try {
      ...
      throw new Error(
        `Cannot access folder ${folderId}: ${folderError.message}`
      );
    }
    ...
    if (error.code === 403) {
      ...
      throw new Error(
        "Folder not found. The folder may have been moved or deleted."
      );
    ...
    throw new Error(
      `Failed to fetch folder files: ${error.message || "Unknown error"}`
    );
  }
}
