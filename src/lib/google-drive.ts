...
class GoogleDriveService {
    ...
    try {
      ...
      if (process.env.VERCEL) {
        ...
        try {
          ...
          console.error('Failed to parse Google credentials on Vercel:', error)
          throw error
        }
      } else {
        // Create service account auth
  ...
  async getFoldersByType(folderType: 'photos' | 'videos'): Promise<DriveFolder[]> {
    ...
    try {
      ...
      console.log(`Searching for ${folderType} folder with query:`, subfolderQuery)
